...
import com.nucleus.web.security.ResponseNoCachingFilter;
...
/**
 * This filter is a copy of <a href="http://tomcat.apache.org/tomcat-7.0-doc/api/org/apache/catalina/filters/ExpiresFilter.html"><i>ExpiresFilter</i></a> provided by Tomcat 7
 * As such all configurations are valid as provided by Tomcat 7 ExpiresFilter
 * Copied to avoid dependency on Tomcat library.
 * @author Nucleus Software Exports Limited
 * 
 */
public class NeutrinoExpiresFilter implements Filter {
    ...
    protected enum DurationUnit {
        DAY(Calendar.DAY_OF_YEAR), HOUR(Calendar.HOUR), MINUTE(Calendar.MINUTE), MONTH(Calendar.MONTH), SECOND(
                Calendar.SECOND), WEEK(Calendar.WEEK_OF_YEAR), YEAR(Calendar.YEAR);
    ...
    private static final Logger  log                                              = LoggerFactory
                                                                                          .getLogger(NeutrinoExpiresFilter.class);
        ...
        if (request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
            ...
            if (response.isCommitted()) {
                if (log.isDebugEnabled()) {
                    log.debug("Response already committed", httpRequest.getRequestURL());
                }
    ...
    protected Date getExpirationDate(XHttpServletResponse response) {
        ...
        if (configuration != null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Use matching configuration", contentType,
                        result);
            }
            return result;
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("No expiration configuration for content type ", contentType);
        }
        return null;
    }
    ...
    protected Date getExpirationDate(ExpiresConfiguration configuration, XHttpServletResponse response) {
        ...
        switch (configuration.getStartingPoint()) {
            ...
            case LAST_MODIFICATION_TIME:
                ...
                break;
            default:
                throw new IllegalStateException("Unsupported starting point");
        }
        ...
        for (Enumeration<String> names = filterConfig.getInitParameterNames() ; names.hasMoreElements() ;) {
            ...
            try {
                if (name.startsWith(PARAMETER_EXPIRES_BY_TYPE)) {
                    ...
                    log.warn("expiresFilter.unknownParameterIgnored", name, value);
                }
            } catch (RuntimeException e) {
                throw new ServletException("expiresFilter.exceptionProcessingParameter" +name +" "+value, e);
            }
        }

        log.debug("Filter initialized", this.toString());
    }
    ...
    protected boolean isEligibleToExpirationHeaderGeneration(HttpServletRequest request, XHttpServletResponse response) {
        ...
        if (expirationHeaderHasBeenSet) {
            if (log.isDebugEnabled()) {
                //log.debug("expiresFilter.expirationHeaderAlreadyDefined", request.getRequestURI(),
        ...
        for (int skippedStatusCode : this.excludedResponseStatusCodes) {
            if (response.getStatus() == skippedStatusCode) {
                if (log.isDebugEnabled()) {
                    log.debug("Skipped status code", request.getRequestURI(),
                            Integer.valueOf(response.getStatus()), response.getContentType());
                }
                return false;
            }
    ...
    public void onBeforeWriteResponseBody(HttpServletRequest request, XHttpServletResponse response) {
        ...
        if (expirationDate == null) {
            if (log.isDebugEnabled()) {
                log.debug("expiresFilter.noExpirationConfigured", request.getRequestURI(),
                        Integer.valueOf(response.getStatus()), response.getContentType());
            }
        } else {
            if (log.isDebugEnabled()) {
                log.debug("expiresFilter.setExpirationDate", request.getRequestURI(),
            ...
            String newCacheControlHeader = (cacheControlHeader == null) ? maxAgeDirective : cacheControlHeader + ", "
                    + maxAgeDirective;
            if(ResponseNoCachingFilter.RESPONSE_CACHE_DISABLED.equals(request.getAttribute(ResponseNoCachingFilter.RESPONSE_CACHE_DISABLED))){
                response.setHeader("Pragma", null); // HTTP 1.0.
                response.setHeader("Expires", null); // Proxies.
            }
    ...
    protected ExpiresConfiguration parseExpiresConfiguration(String inputLine) {
        ...
        try {
            ...
            throw new IllegalStateException("expiresFilter.startingPointNotFound "+line);
        }
        ...
        if ("access".equalsIgnoreCase(currentToken) || "now".equalsIgnoreCase(currentToken)) {
            ...
            throw new IllegalStateException("expiresFilter.startingPointInvalid" + " " +currentToken+" "+line);
        }
        ...
        while (currentToken != null) {
            ...
            try {
                ...
                throw new IllegalStateException("Invalid duration (number) '{}' in directive '{}'"+" "+
                        currentToken +" "+line);
            }
            ...
            try {
                ...
                throw new IllegalStateException("Duration unit not found after amount {} in directive '{}'"+" "+
