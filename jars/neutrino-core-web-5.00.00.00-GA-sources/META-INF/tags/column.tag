...
<%@ attribute name="hidden" required="false" type="java.lang.String" description="If hidden is applicable the column becomes hidden" %>
<%@ attribute name="columnType" required="false" type="java.lang.String" description="If columnType is applicable the column value is displayed as link" %>
...
<c:if test="${htmlVarManagement}">
	...
	<c:choose>
		<c:when test="${title == 'key.actions' || title == 'key.user.actions'}">
			<c:set var="headerCssClasses" value="${headerCssClasses} visibility-exclude" scope="request" />
		</c:when>
		<c:otherwise>
			<c:set var="headerCssClasses" value="${headerCssClasses} visibility-include" scope="request" />
		</c:otherwise>
	</c:choose>
...
<c:if test="${sortManagement}">
	...
	<c:choose>
		<c:when test="${empty hiddenList}">
			<c:set var="hiddenList" value="${hidden eq null or hidden eq 'false' ? false : true}" scope="request" />
		</c:when>
		<c:otherwise>
			<c:choose>
				<c:when test="${hidden eq null or hidden eq false}">
					<c:set var="hiddenList" value="${hiddenList}${delimitor}false" scope="request" />
				</c:when>
				<c:otherwise>
					<c:set var="hiddenList" value="${hiddenList}${delimitor}${hidden}" scope="request" />
				</c:otherwise>
			</c:choose>
		</c:otherwise>
	</c:choose>
