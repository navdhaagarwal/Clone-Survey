...
import com.nucleus.service.NonTransactionalBaseService;
...
public class NonTransactionalMoneyService extends NonTransactionalBaseService implements MoneyService{
    ...
    @Transactional
    public List<Currency> retrieveAllActiveCurrenciesFromDb()
    {
        NamedQueryExecutor<Currency> query = new NamedQueryExecutor<Currency>("Currency.findAllApprovedActiveCurrencies")
        		.addParameter("statusList", getApprovalStatusList()).addQueryHint(QueryHint.QUERY_HINT_CACHEABLE, Boolean.TRUE);
        return entityDao.executeQuery(query);
    }
    ...
    public String formatMoney(String amount, Locale locale) {
        ...
        if (amount != null && !(amount.equals("") && amount != "NaN")) {
            ...
            return MoneyUtils.formatMoneyByLocale(new BigDecimal(amount).toPlainString(), locale, currencyCode);
        } else {
            return null;
        }
    ...
    public Currency getBaseCurrency() {
    	List<Currency> activeCurrencies=retrieveAllActiveCurrencies();
	...
	public String roundAndFormatNumber(BigDecimal unformattedNumber, Map<String,Object> amountMap, RoundingMode roundingMethod) {
		if ((isNull(amountMap.get(PRECISION))) || (isNull(amountMap.get(MULTIPLESOFF)))) {
			amountMap=fetchAmountDetails((Currency)amountMap.get(APPLICABLE_CURRENCY));
		}
	...
	public String getFormattedNumber(BigDecimal rawNumber,Map<String,Object> amountMap) {
		String sign = "";
		int precision=(Integer)amountMap.get(PRECISION);
		BigDecimal unFormattedNumber=rawNumber;
		if (unFormattedNumber == null) {
			unFormattedNumber = BigDecimal.ZERO;
		} else {
			if (unFormattedNumber.compareTo(BigDecimal.ZERO) < 0) {
				unFormattedNumber = unFormattedNumber.negate();
				sign = "-";
			}
		}

		String number = unFormattedNumber.toPlainString();
		...
		groupSeparator = amountMap.get(CONFIG_DIGIT_GROUPING_CONSTANT_CONFIG_KEY).toString();
		decimalSeparator  = amountMap.get(CONFIG_DECIMAL_GROUPING_CONSTANT_CONFIG_KEY).toString();
		amountFormat = amountMap.get(CONFIG_AMOUNT_FORMAT_CONSTANT_CONFIG_KEY).toString();
		
		if (isNull(amountMap.get(CONFIG_NUMBER_FORMAT))) {
			if (notNull(amountFormat)) {
				numberFormat = configurationService
						.getPropertyValueByPropertyKey(amountFormat,
								CONFIGURATION_QUERY);
				if (isNull(numberFormat)) {
					numberFormat = CONFIG_AMOUNT_FORMAT_INDIAN_DESCRIPTION_CONSTANT;
				}
			} else {
				numberFormat = CONFIG_AMOUNT_FORMAT_INDIAN_DESCRIPTION_CONSTANT;
			}
		amountMap.put(CONFIG_NUMBER_FORMAT, numberFormat);	
		} else {
			numberFormat=amountMap.get(CONFIG_NUMBER_FORMAT).toString();
		}		
		numberFormat = numberFormat.replace("C", groupSeparator);
		numberFormat = numberFormat.replace("D", decimalSeparator);
		
		if (decimalSeparator.equals(groupSeparator)) {
			BaseLoggers.flowLogger.error("Decimal separator and digit grouping separator cannot be same.");
			return number.toString();
		}

		if ("#".equals(decimalSeparator) || "#".equals(groupSeparator)) {
			BaseLoggers.flowLogger.error("Decimal separator and digit grouping separator cannot be '#'.");
			return number.toString();
		}

		if (numberFormat.replace("#", "").replace("0", "").replace(decimalSeparator, "").replace(groupSeparator, "").length() > 0) {
			BaseLoggers.flowLogger.error("Invalid number format.");
			return number.toString();
		}
		if(number.contains(decimalSeparator))
		{
			integerPart=number.substring(0, number.indexOf(decimalSeparator));
			decimalPart = number.substring(number.indexOf(decimalSeparator)+1);
		}
		else
		{
			integerPart = number;
		...
		integerPart = formatNumber(integerPart, numberFormat, decimalSeparator, groupSeparator);
		...
		if(precision==0){
			...
			decimalPart ="";
			 ...
			 decimalPart = ""+(Math.pow(10, precision));
			 if(decimalPart.contains(decimalSeparator)){
				 decimalPart = decimalPart.substring(0,decimalPart.indexOf(decimalSeparator));
			 }
			 decimalPart = decimalPart.substring(1);
		 }
		 else if (decimalPart.length() < precision){
			 decimalPart = Math.round(Integer.parseInt(decimalPart) *  Math.pow(10, precision - decimalPart.length()))+"";
		 }
		 else if(decimalPart.length() > precision){
			 decimalPart = decimalPart.substring(0,precision);
		 }
		
		 formattedNumber = sign + integerPart + decimalSeparator+ decimalPart;
		
		return formattedNumber;
	}
	
	protected String formatNumber(String number, String numberFormat, String decimalSeparator, String groupSeparator) {
		String numberStr = number;
		String formattedNumber = "";
		String[] formatArr = numberFormat.split("\\" + decimalSeparator);
		String integerNumberFormat = formatArr[0];

		if (numberStr.length() > integerNumberFormat.replace(groupSeparator, "").length()) {
			BaseLoggers.flowLogger.error("Insufficient number format.");
			return numberStr;
		}

		if (!"".equals(numberStr)) {
			String[] integerNumberFormatArr = integerNumberFormat.split("\\" + groupSeparator);
			for (int i = integerNumberFormatArr.length - 1; i >= 0; i--) {
				if (integerNumberFormatArr[i].length() < numberStr.length()) {
					formattedNumber = numberStr.substring(numberStr.length() - integerNumberFormatArr[i].length(), numberStr.length()) + groupSeparator + formattedNumber;
					numberStr = numberStr.substring(0, numberStr.length() - integerNumberFormatArr[i].length());
				} else if (integerNumberFormatArr[i].length() >= numberStr.length()) {
					formattedNumber = numberStr + groupSeparator + formattedNumber;
					numberStr = "";
					break;
				}
			}
			if (formattedNumber.substring(formattedNumber.length() - 1).equals(groupSeparator)) {
	...
	public Map<String,String> fetchConfigurationProperties() {
		...
		for (Configuration configuration : configurations) {
			...
			if (notNull(configuration.getPropertyKey())
				...
				if (CONFIG_DIGIT_GROUPING_CONSTANT_CONFIG_KEY
				...
				if (CONFIG_DECIMAL_GROUPING_CONSTANT_CONFIG_KEY
						.equals(configuration.getPropertyKey())) {
						amountConfigurations
								.put(CONFIG_DECIMAL_GROUPING_CONSTANT_CONFIG_KEY,
										configuration
				...
				if (CONFIG_AMOUNT_FORMAT_CONSTANT_CONFIG_KEY
						.equals(configuration.getPropertyKey())) {
						amountConfigurations
								.put(CONFIG_AMOUNT_FORMAT_CONSTANT_CONFIG_KEY,
										configuration
